...
namespace Wormhole.Managers
{
    public class WormholeDiscoveryManager : Manager
    {
        ...
        public bool IsLocalGate(string name) => Discoveries.SingleOrDefault(b => b.Value.Gates.Exists(c => c.Name == name)).Key
        ...
        public GateViewModel GetGateByName(string name, out string ownerIp)
        {
            ...
            {
                var gate = discovery.Gates.SingleOrDefault(b => b.Name.Equals(name, StringComparison.OrdinalIgnoreCase));
                if (gate is null)
                    continue;
